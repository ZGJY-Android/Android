 ...
 */
public class MuYingZhuanChangAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>
        implements View.OnClickListener {
    private Context context;
    private ArrayList<ProductBean> myzcProductBean;
    private OnRecyclerViewItemClickListener mOnItemClickListener;

    public MuYingZhuanChangAdapter(ArrayList<ProductBean> myzcProductBean, Context context) {
        this.myzcProductBean = myzcProductBean;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View v;
        v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_muyingzhuanchang,
                parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        listBean = myzcProductBean.get(position);
    ...
    public int getItemCount() {
        // 返回数据有多少条
        if (null == myzcProductBean) {
            return 0;
        }
        // 此处加1是应为item作为header会消耗掉1个长度，导致数据少一个显示
        return myzcProductBean.size();
    }
