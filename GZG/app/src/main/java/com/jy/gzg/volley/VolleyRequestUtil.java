...
public class VolleyRequestUtil {
    ...
    * */
    public static void RequestGet(Context context, String url, String tag, VolleyListenerInterface volleyListenerInterface) {
        // 清除请求队列中的tag标记请求
        RootApplication.getRequestQueue().cancelAll(tag);
        // 创建当前的请求，获取字符串内容
        ...
        // 重启当前请求队列
        RootApplication.getRequestQueue().start();
    }

    /*
    因为后台是post的  但是请求数据又拼在url里  所以没有请求参数
    ...
    * volleyListenerInterface：VolleyListenerInterface接口；
    * */
    public static void RequestPost(Context context, String url, String tag, VolleyListenerInterface volleyListenerInterface) {
        // 清除请求队列中的tag标记请求
        RootApplication.getRequestQueue().cancelAll(tag);
