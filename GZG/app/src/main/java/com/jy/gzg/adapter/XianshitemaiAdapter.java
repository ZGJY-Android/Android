 ...
 */
public class XianshitemaiAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>
        implements View.OnClickListener {
    private ArrayList<ProductBean> xstmBeanList;
    ...
    public XianshitemaiAdapter(ArrayList<ProductBean> xstmBeanList, Context context) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == ITEM_TYPE_HEADER) {
            v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_xianshitemai_header,
                    parent, false);
            return new HeaderViewHolder(v);
        } else {
            v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_xianshitemai,
                    parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof DemoViewHolder) {
            listBean = xstmBeanList.get(position - 1);
            ...
            //((DemoViewHolder) holder).tv_memo.getBackground().setAlpha(150);// 0-255透明度值
